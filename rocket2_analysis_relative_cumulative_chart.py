...
data = data.sort_values(by="provided_eth", ascending=True)
...
def plot_relative_percentile_stats_with_median_and_cumulative(
    ...
    for percentile in np.arange(start_percentile, end_percentile, step):
        ...
        values_in_percentile = data[
            (data["provided_eth"] > lower_bound) & (data["provided_eth"] <= upper_bound)
        ]
    ...
    fig, ax = plt.subplots(figsize=(80, 30))  # Adjusted figure size to make it bigger
