...
# Load the CSV file
file_path = "rocket_minipools_240210.csv"  # Replace with your file path
data = pd.read_csv(file_path)

# Ensure 'value' is sorted in ascending order for cumulative calculations
data = data.sort_values(by="value_t8", ascending=True)
...
def plot_relative_percentile_stats_with_median_and_cumulative(
    ...
    total_sum = data["value_t8"].sum()
    cumulative_sum = 0  # Initialize cumulative sum

    for percentile in np.arange(start_percentile, end_percentile, step):
        lower_bound = data["value_t8"].quantile(percentile / 100.0)
        upper_bound = data["value_t8"].quantile((percentile + step) / 100.0)
        upper_bounds.append(upper_bound)  # Store the upper bound for labeling

        values_in_percentile = data[
            (data["value_t8"] > lower_bound) & (data["value_t8"] <= upper_bound)
        ]
        percentile_sum = values_in_percentile["value_t8"].sum()
    ...
    fig, ax = plt.subplots(figsize=(60, 30))  # Adjusted figure size to make it bigger
    ...
    plt.tight_layout()
    plt.savefig(
        "rocket_analysis_relative_cumulative_chart_with_upper_bounds.png", dpi=200
    )
    # plt.show()


# Plot stats for each 10-percentile range with cumulative sum and upper bound labels
plot_relative_percentile_stats_with_median_and_cumulative(0, 100, 2)
